...
template <>
inline packed_uint<1>::operator unsigned int() const {
  return m_buf[0];
}

template <>
inline packed_uint<2>::operator unsigned int() const {
  return (m_buf[0] << 8U) | m_buf[1];
}

template <>
inline packed_uint<3>::operator unsigned int() const {
  return (m_buf[0] << 16U) | (m_buf[1] << 8U) | (m_buf[2]);
}

template <>
inline packed_uint<4>::operator unsigned int() const {
